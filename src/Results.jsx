...
function getScore(count){
  let score = Math.round(count/39*100);
...
let colors = ["text-slate-700", "text-red-700", "text-orange-700", "text-green-700", "text-blue-700", "text-violet-700", "text-fuchsia-700", "text-rose-700", "text-cyan-700", "text-teal-700", "text-pink-700", "text-emerald-700", "text-zinc-700"];
...
export default function Results(props) {
  return (
    <div class="container flex flex-col items-center">
      ...
      <p class="font-mono text-3xl mb-4">Студент: <span class={color}>{id}</span></p>
      <p class="font-mono text-2xl mb-2">Количество заданий</p>
      <span class="font-mono text-4xl mb-2 text-blue-600">{39}</span>
      ...
      <div class="font-mono text-4xl"><span class={getScore(count())[0]}>{getScore(count())[1]}</span></div>
    </div>
  )
}
