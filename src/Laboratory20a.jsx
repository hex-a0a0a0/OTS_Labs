...
export default function Laboratory(props) {
  ...
  createEffect(on(() => processed(), () => {
    if (processed()) {
      ...
      while (i <= vmax) {
        ...
        y_arr_2.push(pFM);
        y_arr_3.push(pPM);
        arr.push(<Table first={first.toString()} second={second.toString()} third={third.toString()} fourth={i.toString()}></Table>)
        i = i + 0.5;
      }
      setArray(arr);
  ...
  return (
    <main>
      ...
      <p class="font-mono font-normal text-2xl text-center">Исследуем зависимость вероятностей ошибок ДАМ, ДЧМ, ДФМ от соотношения с/ш.</p>
      <Show when={!processed()}>
        <div class="bg-white flex flex-col items-center m-4">
          <button className="submit w-48 font-mono text-extrabold text-slate-50 px-6 py-2 m-2 bg-slate-950 hover:bg-slate-800 rounded-full shadow-lg mt-4" onClick={callback}> Запустить расчет
          </button>
        </div>
      </Show>
      <Show when={processed()}>
        <div class="container mx-auto bg-white flex flex-row items-center m-4">
          <Table first={"ДАМ N_ош"} second={"ДЧМ N_ош"} third={"ДФМ N_ош"} fourth={"h"}></Table>
          {array()}
        </div>
        <div id="plot"></div>
      </Show>
